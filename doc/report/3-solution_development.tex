...
\noindent
Table \ref{tab:f-values} reports few reference values of the function\footonote{The values of the function may seem too much low (e.g., p(10) = 0.15\%). However, we must remind that when reaching time x, the transition has already been tested x-1 times at the previous steps, and so the probability the transition happens at most at time t should be calculated as the sum of the transition to happen in all }.
